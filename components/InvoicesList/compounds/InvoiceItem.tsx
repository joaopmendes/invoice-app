  ...
  return (
    <section>
      ...
      <div
        key={invoice.id}
        className={
          'hidden tablet:flex w-full min-h-[72px] bg-pure-white my-4 rounded-3xl px-8 gap-8  justify-center items-center dark:bg-[#1E2139]'
        }
      >
      ...
      <div
        key={invoice.id}
        className={
          'block tablet:hidden w-full min-h-[72px] bg-pure-white my-4 rounded-3xl px-8 gap-8  justify-center items-center dark:bg-[#1E2139]'
        }
      >
...
export const DisplayDate: React.FC<MiniComponentsProps> = ({ invoice, className }) => {
  return (
    <Typography
      ...
      className={clsx('break-words w-32', className)}
    >
      Due{' '}
      {fns.format(fns.addDays(new Date(invoice.invoiceDate), invoice.paymentTerms), 'dd MMM yyyy')}
    </Typography>
  );
};
...
export const DisplayPrice: React.FC<MiniComponentsProps> = ({ invoice, className }) => {
  return (
    <Typography
      tag={'div'}
      color={'dark-grey'}
      size={'heading-s-variant'}
      className={clsx('break-words w-16', className)}
    >
...
export const DisplayStatus: React.FC<MiniComponentsProps> = ({ invoice, className }) => {
  return <InvoiceStatus status={invoice.status} className={clsx('', className)} />;
};
