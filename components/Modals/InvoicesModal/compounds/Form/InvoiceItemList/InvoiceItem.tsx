  ...
  return (
    <div key={field.id} className={'flex flex-wrap tablet:flex-nowrap gap-2'}>
      <FormItem className={'flex-grow w-full  tablet:w-auto'}>
        <FormLabel
          smaller
          className={clsx({ [`tablet:hidden`]: index != 0 }, 'dark:text-light-blue')}
        >
          Name
        </FormLabel>
        <FormControl>
          <Input
            key={field.id}
            {...form.register(`itemList.${index}.name` as const, { required: true })}
          />
        </FormControl>
        <FormMessage />
      </FormItem>
      ...
      <FormItem className={'w-[28%] tablet:w-[100px] desktop:w-[100px]'}>
        ...
        <FormControl>
          <Input
            key={field.id}
            {...form.register(`itemList.${index}.price` as const, {
              valueAsNumber: true,
              required: true,
            })}
          />
